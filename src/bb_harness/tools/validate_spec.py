...
def check_requirements_table(content: str) -> dict[str, Any]:
    ...
    duplicate_ids = sorted({item for item in requirement_ids if requirement_ids.count(item) > 1})
...
def validate_spec(path: Path) -> dict[str, Any]:
    ...
    requirements_ok = (
        result["requirements"]["status"] == "found"
        and result["requirements"].get("total", 0) > 0
        and not result["requirements"].get("duplicate_ids")
    )
    acceptance_ok = (
        result["acceptance"]["status"] == "found"
        and result["acceptance"].get("total", 0) > 0
        and result["acceptance"].get("unchecked", 0) == 0
    )
    result["summary"] = {
        "quality_score": quality_score,
        "section_errors": critical_errors,
        "has_requirements": requirements_ok,
        "has_acceptance": acceptance_ok,
        "acceptance_items": result["acceptance"].get("total", 0),
        "overall": (
            "pass"
            if critical_errors == 0 and quality_score >= 75 and requirements_ok and acceptance_ok
            else "fail"
        ),
    }
