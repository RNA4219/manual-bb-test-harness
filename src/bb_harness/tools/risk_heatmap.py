...
def generate_svg_heatmap(
    ...
    # Title
    svg_parts.append(
        f'  <text class="title" x="{width // 2}" y="20" text-anchor="middle">Risk Heatmap: Impact x Likelihood</text>'
    )
    ...
    svg_parts.append(
        f'  <text class="axis-label" x="{width // 2}" y="{height - 10}" text-anchor="middle">Impact</text>'
    )
    for i in range(5):
        x = margin_left + i * cell_size + cell_size // 2
        svg_parts.append(
            f'  <text class="axis-label" x="{x}" y="{height - 25}" text-anchor="middle">{i + 1}</text>'
        )
    ...
    for i in range(5):
        ...
        svg_parts.append(
            f'  <text class="axis-label" x="40" y="{y}" text-anchor="end">{5 - i}</text>'
        )
    ...
    for risk in risks:
        ...
        svg_parts.append(
            f'  <circle cx="{x}" cy="{y}" r="12" fill="{color}" stroke="#333" stroke-width="1"/>'
        )
        svg_parts.append(
            f'  <text class="risk-label" x="{x}" y="{y + 3}" text-anchor="middle">{risk_id}</text>'
        )
    ...
    for i, (priority, color) in enumerate(PRIORITY_COLORS.items()):
        y = legend_y + 20 + i * 20
        svg_parts.append(
            f'  <rect x="{legend_x}" y="{y}" width="15" height="15" fill="{color}" stroke="#333"/>'
        )
        svg_parts.append(
            f'  <text class="risk-label" x="{legend_x + 20}" y="{y + 12}">{priority}</text>'
        )
...
def generate_html_heatmap(
    ...
    html_parts.append(
        "    .risk-table { margin-top: 30px; border-collapse: collapse; width: 100%; }"
    )
    html_parts.append(
    ...
    html_parts.append(
        "        <tr><th>ID</th><th>Scenario</th><th>Impact</th><th>Likelihood</th><th>Score</th><th>Priority</th><th>Rationale</th></tr>"
    )
    html_parts.append("      </thead>")
