...
class Suite:
    ...
    def save(self):
        (self.lab / "results.json").write_text(json.dumps({
            "target": "bb-harness 4.1.0 from PyPI", "runner_kind": "agent-operated black-box CLI",
...
def run(repo, lab):
    ...
    def resolve(review):
        finding(review, "high")
        review["resolutions"] = [{"issue_id": "F-1", "decision": "この合成試験では先の保存を確定した後に次を保存する。",
    ...
    def repeat():
        ...
        for key in ("input_sha256", "score", "counts", "requirements", "confirmations"):
            require(first.get(key) == second.get(key), f"repeat preserves {key}")
    s.case("BB-29", repeat)
