...
### 4.1.1: 自己BBの入力不具合と分岐率判定の修正

日本語だけのファイル名、UTF-8 BOM付きMarkdown、明示buildの証跡0件について、
既存の意図した動作へ修正する。CIは行・分岐の合算値を分岐率として扱っていた判定を訂正し、
実測分岐率で全体85%・Gate専用90%を検証する。利用者向けartifact契約・採点policy・
ライセンスを変えない互換修正として4.1.1を発行する。CI内部の収集契約v2はspec-08に従う。

README、package、CLI、PowerShell validator、Workflow Cookbookの現行版とテスト件数を同期する。
PRとmainの緑CI後に新規タグv4.1.1からwheel・sdistを作成し、同じ配布物をTrusted Publishingで
公開する。公開後のhash照合と隔離インストールを完了条件とし、既存タグ・配布物は保持する。
